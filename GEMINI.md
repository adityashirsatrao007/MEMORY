...
```
skill-vector-search "<task>" 10    # semantic search (embeddings)
skill-find "<task keyword>" 5      # keyword search (ripgrep, ~10ms)
```
...
| API Key Routing | `21-api-key-routing.md` — which FREE provider to call per use case (load before any model call) |
...
**2769+ skills installed. I auto-discover them per task:**
1. **Vector search** — `skill-vector-search "<task>" 10` — semantic search over all skills (embeddings, best for fuzzy queries)
2. **Keyword find** — `skill-find "<task keyword>" 3` → instant text search across all skills (ripgrep, ~10ms)
3. **Catalog browse** — `cat $MEMORY_ROOT/memory/skills_catalog.md` — compact name+description index
4. **Load only one** — `bat --line-range :80 "$MEMORY_ROOT/.agents/skills/<match>/SKILL.md"` — read first 80 lines, stop if irrelevant
5. **Abort early** — if no skill matches, do the task directly without loading any
...
- **Proactive key use**: Aditya does NOT command which API/provider to use. Before any model task, load `21-api-key-routing.md` and pick the best key yourself (free-first). Never ask which key.
